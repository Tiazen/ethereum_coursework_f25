...
Расширение состоит из 3 слоев:

1) **UI**
- "Главное приложение" расширения
- Здесь выполняются **все крипто-операции**:
  - PBKDF2
  - AES-GCM (шифр/дешифр приватного Ed25519 ключа и кредов)
  - подпись JWT
- UI показывает пользователю:
  - создание и анлок волта
  - список сайтов/кредов
  - окно подтверждения логина (approve/deny)
  - список pending login-запросов (берётся из background через messaging)

2) **Background Script**
- Что-то похожее на сессионный менеджер
- Хранит:
  - `session.isUnlocked`, `autoLockTimeout`, `lastActivity`
  - `pendingLoginRequests: Map<requestId, PendingLoginRequest>` с TTL (~5 минут)
- Делает:
  - `REQUEST_LOGIN`: проверка `callbackUrl` (same-origin с текущей страницей или localhost+порт), создание `requestId`, сохранение pending.
  - `GET_PENDING_LOGIN_REQUESTS`: отдаёт pending запросы UI.
  - `CONFIRM_LOGIN`: принимает `{ requestId, token }`, отправляет токен на `callbackUrl`, затем шлёт в таб `LOGIN_COMPLETE`
  - авто-лок: по таймауту ставит `isUnlocked=false` и рассылает всем табам `VAULT_LOCKED`.

3) **Content Script (+ inpage.js)**
- Мост между страницей и расширением.
- Content script:
  - инжектит `inpage.js` на страницу
  - принимает `window.postMessage` от `inpage.js`
  - проксирует в background через `browser.runtime.sendMessage`
  - принимает `LOGIN_COMPLETE` из background и возвращает результат обратно
- inpage.js:
  - живёт в контексте страницы
  - даёт сайту API для вызова "Login with extension" (через postMessage)
...
5. **Привязка к кошельку**: Токен содержит адрес кошелька (issuer)
